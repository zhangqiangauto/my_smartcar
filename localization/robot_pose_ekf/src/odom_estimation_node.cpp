...
namespace estimation
{
  ...
  void OdomEstimationNode::odomCallback(const OdomConstPtr& odom)
  {
    ...
    //给滤波器增加一个测量更新，因为这里需要的是从base_footprint_frame_-->wheelodom的转换，所以前面的到的odom_meas要反转。 odom_estimation.cpp第304行
  ...
  // filter loop
  //用手写一个函数OdomEstimationNode::spin而不是一般见到的ros自带的spin函数，
  //用来检查各个传感器的使用状态，以及发布我们需要的topic和tf，这个函数将以30Hz的频率运行。
  void OdomEstimationNode::spin(const ros::TimerEvent& e)
  {
    ...
    if (odom_active_ || imu_active_ || vo_active_ || gps_active_){
      ...
      // update filter
      if ( my_filter_.isInitialized() )  // ekf filter  OdomEstimation my_filter_;  class OdomEstimation
      {
        bool diagnostics = true;
                // 这一句if中实际更新滤波器，也是核心语句 
        if (my_filter_.update(odom_active_, imu_active_,gps_active_, vo_active_,  filter_stamp_, diagnostics))
        {
          ...
          pose_pub_.publish(output_);  //  /odom_combined
          ekf_sent_counter_++;
