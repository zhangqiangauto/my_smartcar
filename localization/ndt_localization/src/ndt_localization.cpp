...
bool NDTLocalization::init()
{
    ...
    pnh_.param<bool>("use_imu", param_use_imu, false);
    ...
    tf_btol_ = (tl_btol * rot_z_btol * rot_y_btol * rot_x_btol).matrix( );    // Eigen::Matrix4f tf_btol_;
    // current_map2odom
    ...
    if (param_use_imu) 
        sub_imu_pose = nh_.subscribe<geometry_msgs::PoseStamped>("/imu_current_pose", 100, boost::bind(&NDTLocalization::imuposeCB, this, _1));   
...
void NDTLocalization::initialPoseCB(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg)
{
    ...
    pre_pose_ = current_pose_ = pre_pose_odom_ = current_pose_odom_  = initial_pose_;
    pose_init_ = true;
...
void NDTLocalization::imuposeCB(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    imu_pose_initial = true;
    current_imu_pose.x = msg->pose.position.x;
    current_imu_pose.y = msg->pose.position.y;
    tf::Quaternion tmp_q;
    tf::quaternionMsgToTF(msg->pose.orientation, tmp_q);
    tf::Matrix3x3(tf::Quaternion(tmp_q)).getEulerYPR(current_imu_pose.yaw, current_imu_pose.pitch, current_imu_pose.roll);
   //std::cout<<"imu current pose: "<<current_imu_pose.x<<  current_imu_pose.y<<std::endl;
}
...
void NDTLocalization::pointCloudCB(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
    ...
    // if (param_use_odom_) 
    //     predict_ndt_pose = predict_pose_odom_; //odom预测的位置
    // else 
    //     predict_ndt_pose = pre_pose_;

    if (param_use_imu&&imu_pose_initial) 
        predict_ndt_pose = current_imu_pose; //odom预测的位置
    else 
        predict_ndt_pose = pre_pose_;
  ...
  else if (param_method_type_ == METHOD_OMP) 
   {
    ...
    std::cout<<"predict_pose_error_: "<<predict_pose_error_<<std::endl;
    ...
    if (!use_predict_pose) 
    {
        ...
        std::cout<<"ndt_pose: "<<ndt_pose.x<<"  "<<ndt_pose.y<<std::endl;
    } 
    else if(imu_pose_initial)
    {
        current_pose_ = predict_ndt_pose;
        ROS_WARN("Use imu predict pose");
        std::cout<<"predict_ndt_pose: "<<predict_ndt_pose.x<<"  "<<predict_ndt_pose.y<<std::endl;
    }
    else
    ...
    msg_current_pose_.header.frame_id = param_map_frame_;  //  map
