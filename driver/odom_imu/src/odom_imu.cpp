...
void IMU::poseCB(const geometry_msgs::PoseStampedConstPtr& msg)//给imu提供每个时间段的初始速度 并且给imu提供初始位置
{
    ...
    if(imu_pose_re_initial)  //重新初始化IMU位置
    {
      imu_init_pose.x = msg->pose.position.x;
      imu_init_pose.y = msg->pose.position.y;
      imu_init_pose.z = 1.;
      ...
      ROS_INFO("re-initial imu pose:(%.2f,%.2f,%.2f,%.2f,%.2f,%.2f)",imu_init_pose.x,imu_init_pose.y,imu_init_pose.z,
                                                   imu_init_pose.roll,imu_init_pose.pitch,imu_init_pose.yaw);

    }

    if (q.size() < 5) {   // std::queue<geometry_msgs::PoseStamped> q; 计算ndt速度
        q.push(*msg);
        return;
    }
    first_pose_ = false;
...
void OdomImu::odomCB(const can_msgs::vehicle_status::ConstPtr& msg)
{
    ...
    ndt_vel.header = msg->Header;
    ndt_vel.twist.linear.x = msg->cur_speed;
    // ndt_vel.twist.linear.y = 0;
    // ndt_vel.twist.linear.z = 0;
...
void OdomImu::imuCB(const sensor_msgs::Imu::ConstPtr& msg)
{
    ...
    current_pose_.x += ndt_vel.twist.linear.x * std::cos(current_pose_.yaw) * diff_time;
    current_pose_.y += ndt_vel.twist.linear.x * std::sin(current_pose_.yaw) * diff_time;
    current_pose_.z = 0.;
